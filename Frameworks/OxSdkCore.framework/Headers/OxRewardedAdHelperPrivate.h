//
//  OxRewardedAdHelperPrivate.h
//  OxSDK-Game
//
//  Created by Mavl_2023_100272 on 2025/4/29.
//  Copyright © 2025 耿志向. All rights reserved.
//

#ifndef OxRewardedAdHelperPrivate_h
#define OxRewardedAdHelperPrivate_h

#import "OxAdHelperPrivate.h"
#import "OxRewardedAdHelper.h"
#import "RewardedInternalAdDelegate.h"

NS_ASSUME_NONNULL_BEGIN

@interface OxRewardedAdHelper() <RewardedInternalAdDelegate>

// 缓存
@property (nonatomic, strong) NSMutableArray<OxAd *> *mCacheAds;
@property (nonatomic, assign) BOOL mIsAllFailed;

@property (nonatomic, assign) bool mUserRewarded;
@property (nonatomic, assign) double mOnRewardedVideoStartedTime;

@end

NS_ASSUME_NONNULL_END


#endif /* OxRewardedAdHelperPrivate_h */
