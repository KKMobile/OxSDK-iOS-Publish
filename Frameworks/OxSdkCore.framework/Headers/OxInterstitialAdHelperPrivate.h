//
//  OxInterstitialAdHelperPrivate.h
//  OxSDK-Game
//
//  Created by Mavl_2023_100272 on 2025/4/29.
//  Copyright © 2025 耿志向. All rights reserved.
//

#ifndef OxInterstitialAdHelperPrivate_h
#define OxInterstitialAdHelperPrivate_h

#import "OxAdHelperPrivate.h"
#import "OxInterstitialAdHelper.h"
#import "InterstitialInternalAdDelegate.h"

NS_ASSUME_NONNULL_BEGIN

@interface OxInterstitialAdHelper () <InterstitialInternalAdDelegate>

// 缓存
@property (nonatomic, strong) NSMutableArray<OxAd *> *mCacheAds;
@property (nonatomic, assign) BOOL mIsAllFailed;

@end

NS_ASSUME_NONNULL_END

#endif /* OxInterstitialAdHelperPrivate_h */
